  ...
  override def run(): Unit = {
    while (true) {
      ...
      producer.send(new ProducerRecord[String, String]("edu", jsonString),
        (metadata: RecordMetadata, exception: Exception) => { //回调方法
        if (exception == null) {
...
//  /usr/local/kafka/bin/kafka-topics.sh --delete --zookeeper node1:2181 --topic edu
//  /usr/local/kafka/bin/kafka-topics.sh --create --zookeeper node1:2181 --replication-factor 1 --partitions 3 --topic edu
//  /usr/local/kafka/bin/kafka-console-consumer.sh --bootstrap-server node1:9092 --topic edu --from-beginning
}